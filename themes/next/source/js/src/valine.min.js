  ...
  return function (e) {
    ...
    ! function (i) {
      ...
      function g(e, t) {
        ...
        return r = d(a.concat(f(t)), 512 + 8 * t.length), h(d(o.concat(r), 640))
      }
      ...
      ! function (t) {
        ...
        function h(e, t, r) {
          ...
          if (r || "function" == typeof t) {
            ...
            return t && (t = d({}, h.defaults, t)), o.parse(n.lex(e, t), t)
          } catch (e) {
        ...
        f._label = /(?:\\[\[\]]|[^\[\]])+/, f._title = /(?:"(?:\\"|[^"]|"[^"\n]*")*"|'\n?(?:[^'\n]+\n?)*'|\([^()]*\))/, f.def = c(f.def).replace("label", f._label).replace("title", f._title).getRegex(), f.bullet = /(?:[*+-]|\d+\.)/, f.item = /^( *)(bull) [^\n]*(?:\n(?!\1bull )[^\n]*)*/, f.item = c(f.item, "gm").replace(/bull/g, f.bullet).getRegex(), f.list = c(f.list).replace(/bull/g, f.bullet).replace("hr", "\\n+(?=\\1?(?:(?:- *){3,}|(?:_ *){3,}|(?:\\* *){3,})(?:\\n+|$))").replace("def", "\\n+(?=" + f.def.source + ")").getRegex(), f._tag = "(?!(?:a|em|strong|small|s|cite|q|dfn|abbr|data|time|code|var|samp|kbd|sub|sup|i|b|u|mark|ruby|rt|rp|bdi|bdo|span|br|wbr|ins|del|img)\\b)\\w+(?!:|[^\\w\\s@]*@)\\b", f.html = c(f.html).replace("comment", /<!--[\s\S]*?-->/).replace("closed", /<(tag)[\s\S]+?<\/\1>/).replace("closing", /<tag(?:"[^"]*"|'[^']*'|\s[^'"\/>\s]*)*?\/?>/).replace(/tag/g, f._tag).getRegex(), f.paragraph = c(f.paragraph).replace("hr", f.hr).replace("heading", f.heading).replace("lheading", f.lheading).replace("tag", "<" + f._tag).getRegex(), f.blockquote = c(f.blockquote).replace("paragraph", f.paragraph).getRegex(), f.normal = d({}, f), f.gfm = d({}, f.normal, {
          ...
          for (var n, r, i, a, o, s, l, c, p, u, d; e;)
            ...
            for (e = e.substring(i[0].length), a = i[2], d = a.length > 1, this.tokens.push({
                type: "list_start",
                ordered: d,
                start: d ? +a : ""
          ...
          else if (t && (i = this.rules.def.exec(e))) e = e.substring(i[0].length), i[3] && (i[3] = i[3].substring(1, i[3].length - 1)), p = i[1].toLowerCase(), this.tokens.links[p] || (this.tokens.links[p] = {
            href: i[2],
            title: i[3]
          });
        ...
        v._scheme = /[a-zA-Z][a-zA-Z0-9+.-]{1,31}/, v._email = /[a-zA-Z0-9.!#$%&'*+\/=?^_`{|}~-]+(@)[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)+(?![-_])/, v.autolink = c(v.autolink).replace("scheme", v._scheme).replace("email", v._email).getRegex(), v._inside = /(?:\[[^\[\]]*\]|\\[\[\]]|[^\[\]]|\](?=[^\[]*\]))*/, v._href = /\s*<?([\s\S]*?)>?(?:\s+['"]([\s\S]*?)['"])?\s*/, v.link = c(v.link).replace("inside", v._inside).replace("href", v._href).getRegex(), v.reflink = c(v.reflink).replace("inside", v._inside).getRegex(), v.normal = d({}, v), v.pedantic = d({}, v.normal, {
          ...
          for (var t, n, r, i, a = ""; e;)
            ...
            else if (i = this.rules.autolink.exec(e)) e = e.substring(i[0].length), "@" === i[2] ? (n = s(this.mangle(i[1])), r = "mailto:" + n) : (n = s(i[1]), r = n), a += this.renderer.link(r, null, n);
          ...
          } else i[0] = this.rules._backpedal.exec(i[0])[0], e = e.substring(i[0].length), "@" === i[2] ? (n = s(i[0]), r = "mailto:" + n) : (n = s(i[0]), r = "www." === i[1] ? "http://" + n : n), a += this.renderer.link(r, null, n);
          return a
          ...
          this.options.baseUrl && !m.test(e) && (e = p(this.options.baseUrl, e));
          var i = '<a href="' + e + '"';
        ...
        u.exec = u, h.options = h.setOptions = function (e) {
          ...
          xhtml: !1,
          baseUrl: null
        }, h.Parser = o, h.parser = o.parse, h.Renderer = i, h.TextRenderer = a, h.Lexer = n, h.lexer = n.lex, h.InlineLexer = r, h.inlineLexer = r.output, h.parse = h, e.exports = h
        ...
        return i(e, [{
          ...
          value: function (e) {
            var t = this,
              n = e.av || t.v;
            try {
              var r = "[object HTMLDivElement]" === c.call(e.el) ? e.el : document.querySelectorAll(e.el)[0];
              if ("[object HTMLDivElement]" != c.call(r)) throw "The target element was not found.";
              ...
              var s = t.el.querySelector(".vsmile-icons"),
                p = e.emoticon_list || ["mrgreen", "neutral", "twisted", "arrow", "eek", "smile", "confused", "cool", "evil", "biggrin", "idea", "redface", "razz", "rolleyes", "wink", "cry", "surprised", "lol", "mad", "sad", "exclaim", "question"];
              for (var u in p) {
                var h = document.createElement("img");
                h.setAttribute("src", e.emoticon_url + "/" + p[u]), s.appendChild(h)
              }
              n.init({
              ...
              var f = "https://github.com/panjunwen/Valine/issues";
              return void(t.el ? t.nodata.show('<pre style="color:red;text-align:left;">' + e + "<br>Valine:<b>" + t.version + "</b><br>反馈：" + f + "</pre>") : console && console.log("%c" + e + "\n%cValine%c" + t.version + " " + f, "color:red;", "background:#000;padding:5px;line-height:30px;color:#fff;", "background:#456;line-height:30px;padding:5px;color:#fff;"))
            }
            ...
            g.innerHTML = v, t.loading = {
              show: function () {
                g.setAttribute("style", "display:block;"), t.nodata.hide()
              },
              hide: function () {
                g.setAttribute("style", "display:none;"), 0 === t.el.querySelectorAll(".vcard").length && t.nodata.show()
              }
            };
            ...
            m.innerHTML = v, t.submitting = {
              ...
              hide: function () {
                m.setAttribute("style", "display:none;"), t.nodata.hide()
              }
            };
            var b = t.el.querySelector(".vmark");
            t.alert = {
              show: function (e) {
                b.innerHTML = '<div class="valert txt-center"><div class="vtext">' + e.text + '</div><div class="vbtns"></div></div>';
                ...
                if (n.innerHTML = "" + r + (e.type && i), b.querySelector(".vcancel").addEventListener("click", function (e) {
                    t.alert.hide()
                  }), b.setAttribute("style", "display:block;"), e && e.type) {
            ...
            x.equalTo("url", l.url), x.count().then(function (n) {
          ...
          value: function (e) {
              ...
              if (void 0 !== r) {
                ...
                if (document.selection) n.focus(), sel = document.selection.createRange(), sel.text = i, n.focus();
                  ...
                  var a = n.selectionStart,
                    o = n.selectionEnd,
                    s = o;
                  n.value = n.value.substring(0, a) + i + n.value.substring(o, n.value.length), s += i.length, n.focus(), n.selectionStart = s, n.selectionEnd = s
            ...
            var c = function (e) {
              ...
              u = function () {
                ...
                return e.select(["nick", "comment", "link", "rid", "isSpam", "emailHash"]), e.equalTo("url", l.url), e.addDescending("createdAt"), e
              },
              g = 1;
            ! function e() {
            ...
            var m = function (e) {
                ...
                var i = "d41d8cd98f00b204e9800998ecf8427e" == e.get("emailHash") ? "9e63c80900d106cbbec5a9f4ea433a3e" : e.get("emailHash"),
                  a = "https://gravatar.loli.net/avatar/" + i + "?size=80";
                ...
                for (var o = t.el.querySelector(".vlist"), s = o.querySelectorAll("li"), l = r.querySelector(".vat"), p = r.querySelectorAll("a"), u = 0, d = p.length; u < d; u++) {
                  var h = p[u];
              ...
              x = {};
            for (var w in b) b.hasOwnProperty(w) && function () {
              var e = b[w],
                n = t.el.querySelector("." + w);
              x[e] = n, d.on("input", n, function (t) {
                l[e] = f.encode(n.value.replace(/(^\s*)|(\s*$)/g, ""))
              })
            }();
            var k = function () {
              ...
              if (e) {
                ...
                if ("" != e.mail) {
                  t.el.querySelector(".visitor_avatar").setAttribute("src", "https://gravatar.loli.net/avatar/" + s(e.mail.toLowerCase().trim()) + "?size=80")
                }
              }
            };
            k(), t.reset = function () {
            ...
            var y = t.el.querySelector(".vsubmit"),
              _ = function (n) {
                if (y.getAttribute("disabled")) return void t.alert.show({
                  type: 0,
                  text: '再等等，评论正在提交中ヾ(๑╹◡╹)ﾉ"',
                  ctxt: "好的"
                });
                if ("" == l.comment) return void x.comment.focus();
                if ("" == l.nick && (l.nick = "小调皮"), l.comment = l.comment.replace(/!\(:(.*?\.\w+):\)/g, "![](" + e.emoticon_url + "/$1)"), l.comment = (0, o.default)(l.comment), l.comment.indexOf(l.at) > -1 && "" != l.at) {
                  var r = '<a class="at" href=\'#' + l.rid + "'>" + l.at + "</a>";
                  l.comment = l.comment.replace(l.at, r)
                }
                var i = h.mail(l.mail),
                  a = h.link(l.link);
                l.mail = i.k ? i.v : "", l.link = a.k ? a.v : "", i.k || a.k ? i.k ? a.k ? A() : t.alert.show({
                  type: 1,
              ...
              A = function () {
                y.setAttribute("disabled", !0), t.submitting.show();
                ...
                for (var r in l)
                  if (l.hasOwnProperty(r)) {
                    ...
                    var i = l[r];
                    n.set(r, i)
                  } n.set("emailHash", s(l.mail.toLowerCase().trim())), n.setACL(S()), n.save().then(function (e) {
                  ...
                  n.innerText = Number(n.innerText) + 1, m(e, !0), y.removeAttribute("disabled"), t.submitting.hide(), t.nodata.hide(), t.reset()
                }).catch(function (e) {
                  t.submitting.hide()
                })
              },
              L = function (e) {
                d.on("click", e, function (n) {
                  ...
                  l.rid = i, l.at = r, x.comment.value = r + " ，" + x.comment.value, x.comment.focus(), t.el.querySelector(".comment_trigger").setAttribute("style", "display:none"), t.el.querySelector(".auth-section").removeAttribute("style"), t.el.querySelector(".veditor").focus()
                })
              };
